...
await test('スクラップブック編集はページ選択、保存、失敗時保持、破棄確認を備える', () => {
  ...
  assert.match(scrapbookEditorSource, /useBlocker\(dirty\)/);
  ...
  assert.match(scrapbookSaveBarSource, /disabled=\{!dirty\}/);
...
await test('スクラップブック編集室はページ送りと没入表示を備える', () => {
  ...
  assert.match(scrapbookSaveBarSource, /未保存の変更があります/);
  assert.match(scrapbookSaveBarSource, /保存する/);
