...
await test('スクラップブック閲覧は表紙から旅の余韻まで雑誌型に構成する', () => {
  assert.match(scrapbookViewerSource, /ScrapbookMagazineCover/);
  assert.match(scrapbookViewerSource, /eyebrow="Story"/);
  assert.match(scrapbookViewerSource, /<TripJournalTimeline/);
  assert.match(scrapbookViewerSource, /eyebrow="Scenes"/);
  assert.match(scrapbookViewerSource, /eyebrow="Places"/);
  assert.match(scrapbookViewerSource, /eyebrow="Keepsakes"/);
});

await test('スクラップブック閲覧はPhase2の表紙・ハイライト・テーマ情報を利用する', () => {
  assert.match(scrapbookViewerSource, /coverSettings\?\.photoId/);
  assert.match(scrapbookViewerSource, /highlightPhotoIds/);
  assert.match(scrapbookViewerSource, /scrapbook\.themeId/);
  assert.match(scrapbookViewerSource, /scrapbook\.layoutVariant/);
  assert.match(scrapbookViewerSource, /!page\.isHidden/);
