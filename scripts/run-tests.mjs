...
await test('状態優先順位はunvisited < passed < visited < stayed < lived', () => {
...
await test('訪問制覇率、宿泊制覇率、到達率の計算が正しい', () => {
  const visits = [
    ...
    { prefectureCode: '04', status: 'lived', manualStatus: 'lived', calculatedStatus: 'unvisited' },
  ];
...
await test('日本制覇マップは地図内だけをズームできる', () => {
  ...
  assert.match(mapComponent, /scale:\s*1\.34/);
