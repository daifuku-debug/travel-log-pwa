...
await test('タイムマシン地図は地点選択とタイムライン選択を連動する', () => {
  ...
  assert.match(timeMachinePage, /onSelectEvent=\{setSelectedEventId\}/);
...
await test('日本制覇マップは地図内だけをズームできる', () => {
  assert.match(mapComponent, /onWheel=\{handleWheel\}/);
...
await test('旅行詳細は主要情報・思い出導線・管理操作を共通UIで整理する', () => {
  assert.match(tripDetailPage, /<PageHeader/);
  assert.match(tripDetailPage, /backTo="\/trips"/);
  assert.match(tripDetailPage, /旅行を編集/);
  assert.match(tripDetailPage, /スクラップブックを見る/);
...
await test('旅行作成・編集は共通フォームと項目別エラー、保存中状態を使う', () => {
  ...
  assert.match(tripEditPageSource, /window\.confirm/);
...
await test('ホームは旅行中、次の旅行、最近の旅行の順で優先表示する', () => {
  ...
  assert.match(homePageSource, /<FeaturedTripCard/);
  assert.match(homePageSource, /旅の記録を始めましょう/);
  assert.match(homePageSource, /to="\/trips\/new"/);
  assert.match(homePageSource, /to="\/travel-gacha"/);
  assert.match(homePageSource, /to="\/japan-map"/);
  assert.match(homePageSource, /to="\/time-machine"/);
  assert.match(homePageSource, /<LoadingState variant="skeleton"/);
  assert.match(homePageSource, /<ErrorState error=\{error\}/);
});
...
await test('地図画面は進捗、テキスト凡例、選択情報を共通UIで表示する', () => {
  ...
  assert.match(mapComponent, /aria-pressed=\{selectedCode === code\}/);
