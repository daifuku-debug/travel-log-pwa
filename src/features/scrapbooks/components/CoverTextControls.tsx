...
export function CoverTextControls({ draft, onChange }: {
      ...
      <section className="scrapbook-cover-panel" aria-labelledby="cover-position-heading">
        ...
        <div className="scrapbook-cover-position-grid" role="radiogroup" aria-label="表紙の文字位置">
          {template.capabilities.titlePositions.map((position) => {
            ...
            return (
              <button key={position} type="button" className={`scrapbook-cover-position is-${position}${selected ? ' is-selected' : ''}`} role="radio" aria-checked={selected} onClick={() => onChange({ ...draft, coverTitlePosition: position })}>
